...
export async function GET(request: NextRequest) {
  try {
    ...
    const logs = await prisma.apparatusLog.findMany({
      where,
      include: {
        apparatus: { select: { id: true, name: true, category: true } },
    ...
    return NextResponse.json({ error: "Failed to fetch apparatus logs" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const apparatus = await prisma.scienceApparatus.findFirst({
      where: { id: data.apparatusId, ...(user.schoolId ? { schoolId: user.schoolId } : {}) },
    });
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      switch (data.action) {
        case "broken":
          if (apparatus.available < data.quantity) throw new Error("Not enough available items to mark as broken");
          updateData.available = { decrement: data.quantity };
          updateData.broken = { increment: data.quantity };
          break;
        case "lost":
          if (apparatus.available < data.quantity) throw new Error("Not enough available items to mark as lost");
          updateData.available = { decrement: data.quantity };
          updateData.lost = { increment: data.quantity };
          break;
        case "repaired":
          if (apparatus.broken < data.quantity) throw new Error("Not enough broken items to repair");
        ...
        case "restocked":
          ...
          break;
        case "discarded":
          if (apparatus.broken < data.quantity) throw new Error("Not enough broken items to discard");
          updateData.broken = { decrement: data.quantity };
          updateData.totalQuantity = { decrement: data.quantity };
          break;
      }

      await tx.scienceApparatus.update({ where: { id: data.apparatusId }, data: updateData });

      return tx.apparatusLog.create({
        data: {
          apparatusId: data.apparatusId, action: data.action, quantity: data.quantity,
          notes: data.notes, reportedById: user.id,
